...
class InfiniteDistinctToolCallProvider:
    """Cycles through six distinct, valid, never-duplicate tool-call
    decisions (one per allowed capability, so neither the per-tool cap
    of 2 nor the total 8-call cap is reached within one pass) -- exists
    solely to prove the LangGraph recursion_limit backstop fires for a
    decision provider that would otherwise never voluntarily choose to
    stop. 6 tool calls x 4 transitions/call + 2 fixed (InputGuard,
    LoadSession) = 26 transitions, already past the 25-transition
    ceiling, so the graph never even needs a second pass through the
    cycle."""

    _CYCLE = (
        ("get_weather", {"location": "Istanbul", "date_from": "2026-09-10", "date_to": "2026-09-10"}, "missing_weather_info"),
        ("web_search", {"query": "Hagia Sophia hours"}, "missing_current_info"),
        ("search_flights", {"origin": "BEY", "destination": "IST", "depart_date": "2026-09-10", "passenger_count": 1}, "missing_flight_info"),
        ("search_stays", {"check_in": "2026-09-10", "check_out": "2026-09-15", "guest_count": 1}, "missing_stay_info"),
        ("estimate_fair_price", {"stay_id": "stay_fake_d0_001"}, "needs_fair_price"),
        ("call_istanbul_expert", {"question": "What should I see near Sultanahmet?"}, "missing_local_expertise"),
    )
    ...
    def generate(self, system: str, user: str) -> str:
        action, args, reason = self._CYCLE[self.calls % len(self._CYCLE)]
        self.calls += 1
        return _decision(action, args, reason)
...
def _build(decider, tools=None, cancellation_check=lambda: False, monotonic_clock=None, checkpointer=None):
    kwargs = {}
    if monotonic_clock is not None:
        kwargs["monotonic_clock"] = monotonic_clock
    return build_graph(
        tools or FakeToolExecutor(), decider, cancellation_check=cancellation_check, checkpointer=checkpointer, **kwargs
    )
...
def test_duplicate_action_is_blocked_not_re_executed():
    ...
    decider = ScriptedDecisionProvider([
        ...
        _decision("get_weather", weather_args, "missing_weather_info"),  # identical -- must be skipped, not re-executed
        _decision("synthesize", {}, "all_required_evidence_present"),
    ])
    result = start_session(_build(decider, tools), _request("weather please"), "t-dup")
...
def test_rate_limiting_does_not_create_an_infinite_loop():
    ...
    decider = ScriptedDecisionProvider([
        ...
        _decision("search_flights", same_args),  # duplicate: skipped
        _decision("search_flights", same_args),  # second consecutive duplicate: forced to synthesize
    ])
...
def test_maximum_eight_tool_calls_enforced():
    ...
    decide = _make_decide_node(
        decision_provider=ScriptedDecisionProvider([
            _decision("get_weather", {"location": "Istanbul", "date_from": "2026-09-20", "date_to": "2026-09-20"})
        ]),
...
def test_per_tool_call_cap_of_two_is_enforced_independently_of_the_total_cap():
    ...
    state: PlannerState = {
        ...
        "tool_call_count_by_action": {"get_weather": 2},  # but already at the per-tool cap
        "executed_fingerprints": [],
        "observations": [],
        "trace": [],
...
def test_maximum_25_graph_transitions_enforced_by_recursion_limit():
    decider = InfiniteDistinctToolCallProvider()
